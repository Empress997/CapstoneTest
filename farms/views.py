...
import requests
from farms.forms import ContactForm, NewsLetterForm
from .models import *
import json
...
class HomeView(View):
    def get(self, request):
        ...
        def job():
            ...
            subject = 'Zamboanga Farms Newsletter!' 
            message = render_to_string('store/email.html',{'data':data})
            from_email = settings.EMAIL_HOST_USER 
            to_list = emails
            send_mail(subject,message,from_email,to_list,fail_silently=True)
            schedule.every(2).seconds.do(job)
            return schedule.CancelJob
        return render(request, 'store/index2.html', {'form':form,'newslist':newslist})
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            message = 'Greetings! ' + form.cleaned_data['email_address'] + '\nYou will be updated everyday for the weather forecast, crop recommendation, and little description about the weather. \nPlease contact us for your concerns.'
...
def email_newsletter(request):
    ...
    return render(request, 'store/email.html',data)
